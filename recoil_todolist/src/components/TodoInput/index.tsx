...
import { TodoItemTypes, inputState, todosState } from "../../recoil/todo";
import { ChangeEvent, useCallback } from "react";

const TodoInput = ()  => {
    const [contents, setContents] = useRecoilState<string>(inputState);

    const todos = useRecoilValue<TodoItemTypes[]>(todosState);
    const setTodos = useSetRecoilState<TodoItemTypes[]>(todosState);
    ...
    return (
      <div>
        <input
          ...
          onKeyDown={onKeyDown}
        />
        <button className='TodoInput-Button' onClick={addTodo}>추가</button>
      </div>
    )
}
